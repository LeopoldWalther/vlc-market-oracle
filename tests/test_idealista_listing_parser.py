...
# Task 1.5 moved the parser onto the crawl context: operation, municipality, district,
# observed_at and extraction_version now come from the caller rather than the page.
CONTEXT = dict(
    operation="sale",
...
class TestParseListingHtml(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.listing: ListingDetail = parse_listing_html(
            FIXTURE_PATH.read_text(encoding="utf-8"), LISTING_URL, **CONTEXT
        )
    ...
    def test_location_and_coordinates_are_parsed(self) -> None:
        ...
        self.assertEqual(self.listing.page_district, "Ejemplo Norte")
...
class TestParseListingHtmlWithMissingFields(unittest.TestCase):
    def test_an_empty_page_is_rejected_instead_of_returned_half_filled(self) -> None:
        # Contract change in task 1.5: a record missing a required field is quarantined rather
        # than written with None, which used to look like a healthy but empty listing.
