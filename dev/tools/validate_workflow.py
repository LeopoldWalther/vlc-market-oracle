...
def _expected_marker(task_statuses: list[str]) -> str:
    if not task_statuses or all(status == "not_started" for status in task_statuses):
...
def _check_plan(plan: PlanProgress, readme: dict[str, str], errors: list[str], warnings: list[str]) -> None:
    ...
    done_count = sum(status == "done" for status in plan.task_statuses)
    expected = _expected_marker(plan.task_statuses)
    progress = f"{done_count}/{plan.task_count}"
